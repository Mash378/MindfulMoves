...
#Load model from bath
from pathlib import Path
MODEL_PATH = str(Path(__file__).parent / "demo_model").replace("\\", "/")
...
# Standardize moves to uci format
...
def predict_next_move(
    ...
    for attempt in range(max_retries):
        ...
        new_tokens_out = output[0][input_length:]
        generated      = tokenizer.decode(new_tokens_out, skip_special_tokens=True).strip()
        move = ""
        for length in (5, 4):
        ...
        if not move:
            for token in generated.split():
                if is_valid_uci(token):
                    move = token
                    break
        #Keep generating until valid move
        if not move:
            continue
